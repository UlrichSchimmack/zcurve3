#' Plot a fitted z-curve
#'
#' Draws the histogram of significant z-values, the fitted density, and the
#' estimate block (ODR / EDR / ERR / FDR).
#'
#' @param x A `zcurve` object from [zcurve()].
#' @param histogram_col Colour of the histogram bars.
#' @param curve_col Colour of the fitted density curve.
#' @param histogram_width Histogram bar width.
#' @param local_interval z-interval width for the local-power annotations.
#' @param xlim,ylim Axis ranges (`ylim = c(0, ymax)` sets the top of the y-axis).
#' @param ylab y-axis label.
#' @param show_text Logical; show the ODR / EDR / ERR / FDR estimate block.
#' @param show_local_power Logical; annotate local power under the curve.
#' @param line_width Fitted-curve thickness.
#' @param text_size Multiplier for text and point sizes.
#' @param show_ci_band Logical; draw the pointwise bootstrap density band (needs
#'   `boot_iter > 0`).
#' @param show_edr_band Logical; overlay the fitted curves at the extreme EDR
#'   bootstrap replicates.
#' @param ci_level Band / interval level.
#' @param ci_band_col,edr_band_col Band colours.
#' @param ... Further graphical arguments passed to the underlying plot.
#'
#' @return `x`, invisibly; called for the plot it draws. Options stored on the
#'   fit via `zcurve(plot_control = ...)` are used as defaults, and explicit
#'   arguments here override them.
#' @seealso [zcurve()], [zcurve_forest()]
#' @examples
#' fit <- zcurve(zval = abs(rnorm(2000, 2.5)), show_plot = FALSE)
#' plot(fit, ylim = c(0, 0.5))
#' @export
plot.zcurve <- function(
  ...
  # Apply any plot options stored on the fit via zcurve(plot_control = ...) for
  # arguments the caller did not pass explicitly. Args given directly to plot()
  # always win, so the stored look is a default, not a lock.
  if (!is.null(x$plot_control) && length(x$plot_control)) {
    .supplied <- names(as.list(match.call())[-1L])
    .valid    <- setdiff(names(formals()), c("x", "..."))
    for (.nm in names(x$plot_control)) {
      if (!(.nm %in% .supplied) && .nm %in% .valid) assign(.nm, x$plot_control[[.nm]])
    }
  }
  ...
  # Component power for the local-power annotation: two-sided (P(|Z| > crit)) for
  # a folded / nondirectional fit; one-sided in the PREDICTED (positive)
  # direction for a directional fit. In the directional case a study observed in
  # the negative tail then shows its (small) chance of replicating significant in
  # the predicted direction -- a sign error -- instead of a mirrored two-sided
  # power that rises symmetrically into the negative range.
  local_component_power <- if (folded) {
    stats::pnorm(-critical_value, mean = local_ncp, sd = 1) +
      stats::pnorm(critical_value, mean = local_ncp, sd = 1, lower.tail = FALSE)
  } else {
    stats::pnorm(critical_value, mean = local_ncp, sd = 1, lower.tail = FALSE)
  }

  # Mirror the density about 0 only for a nondirectional (folded) analysis, the
  # same rule the fitted curve uses above. A directional fit has signed
  # components, so folding here would let the +z side's high-ncp components
  # explain a negative z, making local power symmetric about 0 and (wrongly)
  # *rise* into the negative tail instead of falling toward the null.
  local_component_density <- vapply(
    seq_along(local_weights),
    function(j) {
      d <- stats::dnorm(density_grid, mean = local_ncp[j], sd = 1)
      if (folded) {
        d <- d + stats::dnorm(-density_grid, mean = local_ncp[j], sd = 1)
      }
      d
    },
    numeric(length(density_grid))
...
#' Minimum-effect forest plot of individual studies
#'
#' Draws, for each individual study, its shrinkage-adjusted effect size (the
#' observed effect pulled toward the fitted mixture) with a credible interval,
#' alongside its adjusted power (or observed z) and the minimum-effect line.
#' Requires a bootstrapped fit (`boot_iter > 0`) with `yi`/`sei` supplied, so the
#' adjusted-effect intervals reflect the uncertainty in the fitted population,
#' not just each study's own standard error.
#'
#' Columns drawn: an optional id column, the study labels, the forest (adjusted
#' effect with its interval, observed effect as an open circle, and the
#' minimum-effect line), and an estimates column (adjusted power or z, then the
#' adjusted effect with its interval). The forest panel is auto-sized; long
#' labels are truncated to keep it at least `forest_frac` of the width.
#'
#' @param object A `zcurve` object fit with `yi`/`sei` and `boot_iter > 0`.
#' @param min_effect Minimum effect of interest (the dashed line and
#'   `P(|d| > min_effect)`). Can be changed here without refitting; defaults to
#'   the fit's `min_effect`.
#' @param z_min Show only studies with `|z| > z_min`.
#' @param es_lb_min,es_lb_max Keep only studies whose adjusted-effect lower bound
#'   lies in `[es_lb_min, es_lb_max]`.
#' @param k_max Show at most this many studies (the top `k_max` by `sort_by`).
#' @param sort_by Sort key: one of `"lower_bound"`, `"prob_meaningful"`,
#'   `"adjusted_effect"`, `"z"`, `"study_id"`.
#' @param decreasing Logical; sort direction.
#' @param show_raw Logical; draw observed (unadjusted) effects as open circles.
#' @param xlim Effect-axis range (auto when `NULL`).
...
#' @param stat Statistic shown beside each effect: `"power"` (adjusted
#'   replication probability, the default) or `"z"` (observed z).
#' @param label_max_frac Cap on the label column as a fraction of figure width.
#' @param forest_frac Minimum share of the figure width reserved for the forest
#'   panel; labels are truncated to keep it.
#' @param text_size Multiplier for all row/header text and point sizes.
#' @param panel_widths Fallback relative widths (labels, forest, estimates), used
#'   only when the automatic column sizing is unavailable.
#' @param file Optional output path. When given, the function opens a device
#'   auto-sized to the study count (so rows stay readable) and writes the plot;
#'   the format comes from the extension (`png`, `jpeg`, `tiff`, `pdf`, `svg`).
#' @param width,height Device size in inches for `file` output (height auto-sizes
#'   to the number of rows when `NULL`).
#' @param res Resolution (pixels per inch) for raster `file` output.
#' @param row_height Inches per row for the auto-sized height (`NULL` derives it
#'   from `text_size`).
#' @param ... Further graphical arguments.
#'
#' @return The plotted data frame (including `study_id` and `cluster_id`),
#'   invisibly.
#' @seealso [zcurve()], [plot.zcurve()]
#' @examples
#' set.seed(1)
#' fit <- zcurve(yi = abs(rnorm(300, 0.5, 0.3)), sei = runif(300, 0.1, 0.2),
#'               boot_iter = 100, show_plot = FALSE,
#'               control = zcurve_control(parallel = FALSE))
#' zcurve_forest(fit, z_min = 3, es_lb_min = 0.1)
#' @export
zcurve_forest <- function(
  ...
  if (!is.null(file) && (length(file) != 1L || !is.character(file) || !nzchar(file))) {
    stop("file must be NULL or a single non-empty path (e.g. \"forest.png\").")
  }
  for (.arg in c("width", "height", "row_height")) {
    .v <- get(.arg)
    if (!is.null(.v) && (length(.v) != 1L || !is.finite(.v) || .v <= 0)) {
      stop(.arg, " must be NULL or one positive number of inches.")
    }
  }
  if (length(res) != 1L || !is.finite(res) || res <= 0) {
    stop("res must be one positive number (pixels per inch for raster files).")
  }
  ...
  # ---- Auto-size the canvas to the number of rows -------------------------
  # A fixed device crams every study into the same height, so with many studies
  # the rows collapse into each other. Sizing only works when the function OPENS
  # the device, i.e. when saving with file = ...: each row then gets a fixed
  # physical height (row_in) and the text stays readable, so the total height
  # grows with the study count. width/height/res tune that file. Drawing into an
  # already-open device cannot be resized from here (a base-R limitation), so we
  # keep the old shrink-to-fit and, if it would be cramped, advise saving.
  row_in   <- if (!is.null(row_height)) row_height else 0.185 * text_size + 0.02
  auto_h   <- 0.75 + n_studies * row_in + 0.95          # header + rows + x-axis
  auto_w   <- if (!is.null(width)) width else 8
  open_dev <- !is.null(file)

  if (open_dev) {
    H   <- if (!is.null(height)) height else auto_h
    W   <- auto_w
    ext <- tolower(tools::file_ext(file))
    switch(ext,
      png          = grDevices::png(file, width = W, height = H, units = "in", res = res),
      jpg = , jpeg = grDevices::jpeg(file, width = W, height = H, units = "in", res = res),
      tif = , tiff = grDevices::tiff(file, width = W, height = H, units = "in", res = res),
      pdf          = grDevices::pdf(file, width = W, height = H),
      svg          = grDevices::svg(file, width = W, height = H),
      stop("unsupported file extension '", ext, "'; use png, jpeg, tiff, pdf, or svg."))
    message(sprintf("Saving %d-study forest to %s (%.1f x %.1f in).",
                    n_studies, file, W, H))
    row_cex <- 0.82 * text_size            # canvas sized to rows -> keep text large
  } else {
    if (!is.null(height) || !is.null(width) || !is.null(row_height)) {
      warning("width/height/row_height only take effect when saving with file = ...; ",
              "an already-open device cannot be resized from here. ",
              "Use e.g. zcurve_forest(fit, file = \"forest.png\") to control the size.",
              call. = FALSE)
    }
    row_cex <- max(.55, min(.95, 22 / n_studies)) * text_size
    dev_h <- tryCatch(graphics::par("din")[2], error = function(e) NA_real_)
    if (is.finite(dev_h) && n_studies >= 40 && dev_h < 0.85 * auto_h) {
      message(sprintf(
        "%d studies are cramped at the current device height (%.1f in). Save with zcurve_forest(fit, file = \"forest.png\") to auto-size the height (about %.0f in).",
        n_studies, dev_h, auto_h))
    }
  }
  ...
  if (open_dev) on.exit(grDevices::dev.off(), add = TRUE)
...
#' Forest-plot alias for zcurve objects
#'
#' A convenience alias for [zcurve_forest()]; `forest.zcurve(fit)` is equivalent
#' to `zcurve_forest(fit)`. (There is no bare `forest()` generic in this package,
#' so call `zcurve_forest()` or `forest.zcurve()` directly.)
#'
#' @param x A `zcurve` object.
#' @param ... Arguments passed to [zcurve_forest()].
#' @return The plotted data frame, invisibly.
#' @seealso [zcurve_forest()]
#' @export forest.zcurve
