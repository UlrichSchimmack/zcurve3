# zcurve3: core model and estimation.
#
# Dependencies (parallel, KernSmooth, stats, graphics, grDevices, utils, tools)
# are declared in DESCRIPTION (Imports) and NAMESPACE, not attached with
# library(). KernSmooth::bkde is imported by name; everything else is called
...
#' Tuning parameters for [zcurve()]
#'
#' Collects fitting, bootstrap, and effect-size tuning options into a list to be
#' passed to `zcurve(control = ...)`.
#'
#' @param boot_iter Bootstrap replicates (500+ for final models). Set here **or**
#'   on [zcurve()], not both.
#' @param parallel Logical; run the bootstrap in parallel. Default `TRUE`.
#' @param cores Number of worker processes, or `NULL` to auto-pick a conservative
#'   count.
#' @param seed Optional integer seed for a reproducible bootstrap.
#' @param tol_criterion EM convergence tolerance.
#' @param max_iter,max_iter_boot EM iteration caps for the point fit and for each
#'   bootstrap replicate.
#' @param ci_alpha Alpha for reported confidence intervals (`0.05` = 95%).
#' @param augment,n_bars Density-method (OF) tuning: lower-bound bias correction
#'   and the density-estimation resolution.
#' @param density_step,bootstrap_density_step Numerical density-grid step for the
#'   point fit and (coarser) for the bootstrap fits.
#' @param bw_est,spline_width Kernel bandwidth and the blend of truncated and
#'   normal kernel densities for the OF fit.
#' @param floor_correct Logical; remove the homogeneous-null floor from the
#'   effect-size heterogeneity `es_tau_sig`. Default `TRUE`.
#' @param floor_reps Homogeneous-null refits averaged to estimate that floor.
#' @param es_null_share Individual-effects prior only: give the null component
#'   (ncp 0) at least this share of the low-power (ncp <= 1) mass, so
#'   genuinely-null studies can shrink to 0. Applied to the population weights for
#'   the individual posterior only; the fit's EDR/ERR/FDR are untouched. `0`
#'   disables.
#'
#' @return A named list of control parameters for `zcurve(control = ...)`.
#' @seealso [zcurve()]
#' @export
...
#' Fit a z-curve to significant test statistics
#'
#' Fits a distribution-free mixture of non-central components to the significant
#' test statistics in a literature and returns power, publication-bias, and (when
#' effect sizes are supplied) effect-size estimates: the observed and expected
#' discovery rates (ODR, EDR), the expected replication rate (ERR), and the
#' false-discovery rate (FDR).
#'
#' Supply exactly one consistent combination of test statistics via `zval`,
#' `tval` + `df`, `pval`, or `yi` + `sei`. `yi`/`sei` may be added to any of the
#' others and are **required** for the effect-size table and the forest plot.
#'
#' @param zval Numeric vector of z-values (the native input).
#' @param tval Numeric vector of t-values; requires `df`.
#' @param df Degrees of freedom for `tval` (scalar or vector).
#' @param pval Numeric vector of two-sided p-values (converted to z internally).
#' @param n Sample sizes, used with some inputs to derive standard errors.
#' @param yi Observed effect sizes. Enables the effect-size estimates and the
#'   forest plot; z is taken as `yi / sei` when test statistics are not otherwise
#'   supplied.
#' @param sei Standard errors of `yi`.
#' @param study_id Optional identifier labelling each individual test (one forest
#'   row per test).
#' @param cluster_id Optional identifier grouping dependent tests; the bootstrap
#'   then resamples whole clusters so dependence does not deflate the intervals.
#' @param est_method Estimation method: `"OF"` (density/KDE, the default),
#'   `"EM"` (likelihood mixture), or `"SQP"`.
#' @param directional Logical; if `TRUE` the sign of the test statistic is
#'   retained. Default `FALSE`.
#' @param folded Logical; if `TRUE` (default) input is folded to `|z|` so both
#'   significant tails inform the fit.
#' @param alpha Significance threshold. Default `0.05`.
#' @param crit Two-sided critical value; derived from `alpha`.
#' @param int_beg Lower bound of the selection window; derived from `crit`.
#'   Negative `ncp` components are only identified when `int_beg <= 0` (no
#'   selection).
#' @param int_end Right-truncation boundary of the modelling grid; statistics
#'   beyond it are treated as power 1.
#' @param edr_ci_adj,err_ci_adj Small adjustments to the EDR / ERR confidence
#'   bounds.
#' @param min_effect Smallest effect size of practical interest (the forest
#'   plot's minimum-effect line and `P(|d| > min_effect)`).
#' @param min_int Minimum number of significant test statistics required to fit.
#'   Default `20`; lower it (with caution — estimates get noisy) to fit a small
#'   set.
#' @param ncp Component locations (non-centralities). Default `0:6`.
#' @param k_ncp Number of components; defaults to `length(ncp)`.
#' @param z_sd Component standard deviations, one per `ncp`, each `>= 1`.
#' @param z_sd_fixed,ncp_fixed,w_fixed Logicals holding the component SDs,
#'   locations, or weights fixed during EM estimation.
#' @param x_lim_min,x_lim_max Lower/upper bound of the modelling grid (and the
#'   plot x-axis).
#' @param int_loc Bin width for local-power / local-effect reporting; `0` disables
#'   it.
#' @param version,date Strings shown on the plot; set to `NULL` or `""` to
#'   suppress.
#' @param show_plot Logical; draw the fitted density automatically. Default
#'   `TRUE`.
#' @param show_summary Logical; print the summary immediately.
#' @param boot_iter Bootstrap replicates (500+ for reported intervals; required
#'   for the forest plot). Set here **or** in [zcurve_control()], not both.
#' @param control A list of tuning parameters from [zcurve_control()].
#' @param plot_control A list of [plot.zcurve()] options used for the automatic
#'   plot and stored on the fit, so later `plot(fit)` calls reuse the same look;
#'   an explicit `plot()` argument still overrides.
#'
#' @return An object of class `"zcurve"`: a list with the fitted estimates (ODR,
#'   EDR, ERR, FDR) and mixture components, plus -- when `yi`/`sei` were supplied
#'   -- an `effect_size` list and per-study `individual_effects`. Has
...
zcurve <- function(
  ...
  min_int      = 20,              # minimum significant tests required to fit (lower for small sets)
  ...
  ### PLOT OPTIONS
  # A list of any plot.zcurve() options (e.g. list(ylim = c(0, 0.9),
  # histogram_width = 0.15)). Used for the automatic plot drawn by
  # show_plot = TRUE and stored on the fit, so later plot(fit) calls reuse the
  # same look; an explicit plot(fit, ...) argument still overrides.
  plot_control = list()
) {
...
if (!is.list(plot_control)) {
  stop("plot_control must be a list of plot() options, e.g. list(ylim = c(0, 0.9), histogram_width = 0.15).")
}
if (length(plot_control) > 0L) {
  pc_nms <- names(plot_control)
  if (is.null(pc_nms) || any(!nzchar(pc_nms))) {
    stop("every plot_control element must be named; they are passed to plot() by name.")
  }
  if (exists("plot.zcurve", mode = "function")) {
    valid_pc <- setdiff(names(formals(plot.zcurve)), c("x", "..."))
    bad_pc   <- setdiff(pc_nms, valid_pc)
    if (length(bad_pc)) {
      stop("unknown plot_control option(s): ", paste(bad_pc, collapse = ", "),
           ". See ?plot.zcurve for the available options.")
    }
  }
}
...
# Negative components are only identified WITHOUT selection. Under selection
# (int_beg > 0) the fit conditions on the significant tail, which carries no
# information to separate ncp = 0 from ncp < 0, so a negative-ncp grid is an
# unidentified fit whose answer is driven by the grid, not the data. Refuse it,
# and point to the no-selection form (open the window down to the components).
if (any(ncp < 0) && isTRUE(int_beg > 0)) {
  stop(sprintf(paste0(
    "Negative ncp components (min(ncp) = %g) are not identified under selection ",
    "(int_beg = %g > 0): the model conditions on the significant tail and cannot ",
    "separate ncp = 0 from ncp < 0. To model negative effects, run WITHOUT ",
    "selection by opening the window down to the components, e.g. int_beg = min(ncp) = %g."),
    min(ncp), int_beg, min(ncp)))
}
...
get_estimates = function(
  ...
  compute_power <- function(est_inp, int_beg, int_end, grid,
    ...
    # Directional fits use one-sided predicted-direction power (pow_pos) so a
    # study in the negative tail shows its chance of replicating significant in
    # the predicted direction (a sign error), not the two-sided total. EDR/ERR
    # above are unaffected -- they use pow_ext / pow_dir_ext.
    pow_local <- if (directional) pow_pos else pow
    pow_grid <- as.vector((lik_mat %*% (w_all * pow_local)) / w_grid)
  ...
  curve_fitting <- function(theta) {
    ...
    if (testing && runif(1) > 0.6) {
      plot(d_x, o_d_y * scale, type = 'l',
           ...
           ylim = c(0, max(c(o_d_y, e_d_y) * scale, na.rm = TRUE)),
      ...
      if (est_method == "OF") {
         ...
         if (!is.null(cluster_id)) {
          ...
          if (!is.null(yi_boot)) {
            ...
            fit_res$es_re_mu <- unname(selection_fit["mu"])
      ...
      if (est_method == "EM") {
         ...
         if (!is.null(cluster_id)) {
          ...
          if (!is.null(yi_boot)) {
            ...
            fit_EM$es_re_mu <- unname(selection_fit["mu"])
 ...
 if (ncp_fixed && z_sd_fixed) {
   ...
   if (est_method == "EM") { 
    ...
    if (!is.null(yi)) {
      ...
      res_pe$es_re_mu_pe <- unname(selection_fit["mu"])
  ...
  if (boot_iter > 0) {
       ...
       boot_res <- run_bootstrap(
          ...
          if(!is.null(yi)) {
            ...
            # The selection-corrected normal RE fit gives a coherent (mu, tau)
            # per replicate. Pooling N(mu_b, tau_b) across replicates folds the
            # sampling error in BOTH mu and tau into the spread, on top of the
            # heterogeneity itself. Folded -> magnitude scale (lower bound >= 0,
            # i.e. cannot fall below "no effect").
            es_re_mu_boot = sapply(boot_res, function(x) x$es_re_mu)
            pi_ok = is.finite(es_re_mu_boot) & is.finite(es_tau_boot)
            if (sum(pi_ok) >= 2L) {
              n_draw = max(1L, ceiling(5000 / sum(pi_ok)))
              pred_draw = rnorm(sum(pi_ok) * n_draw,
                                mean = rep(es_re_mu_boot[pi_ok], each = n_draw),
                                sd   = rep(es_tau_boot[pi_ok],   each = n_draw))
              if (folded) pred_draw = abs(pred_draw)
              pi_ci = unname(quantile(pred_draw, probs = ci_probs, na.rm = TRUE))
            } else {
              pi_ci = c(NA_real_, NA_real_)
            }
            # heterogeneity-only interval: the fitted 95% range of true effects
            # with (mu, tau) treated as known -- wide even with no sampling error.
            # Same folding convention as the bootstrap version above.
            if (is.finite(res_pe$es_re_mu_pe) && is.finite(res_pe$es_tau_pe)) {
              het_draw = rnorm(5000L, res_pe$es_re_mu_pe, res_pe$es_tau_pe)
              if (folded) het_draw = abs(het_draw)
              pi_het = unname(quantile(het_draw, probs = ci_probs, na.rm = TRUE))
            } else {
              pi_het = c(NA_real_, NA_real_)
            }
            prediction_interval = c(
              mu = res_pe$es_re_mu_pe, tau = res_pe$es_tau_pe,
              lb = pi_ci[1], ub = pi_ci[2],
              lb_het = pi_het[1], ub_het = pi_het[2]
            )
        ...
        if (!is.null(yi)) {
          ...
          # heterogeneity-only prediction interval (no bootstrap -> no estimation
          # uncertainty available; lb/ub left NA, het bounds from the point fit)
          if (is.finite(res_pe$es_re_mu_pe) && is.finite(res_pe$es_tau_pe)) {
            het_draw = rnorm(5000L, res_pe$es_re_mu_pe, res_pe$es_tau_pe)
            if (folded) het_draw = abs(het_draw)
            pi_het = unname(quantile(het_draw, probs = ci_probs, na.rm = TRUE))
...
#################################################
  ...
  if (length(min_int) != 1L || !is.finite(min_int) || min_int < 1) {
    stop("min_int must be one positive number.")
  }
  n_tests <- length(test_stat)
  n_sig   <- sum(test_stat > crit)
  if (n_sig < min_int) {
    stop(sprintf(paste0("Insufficient data: need at least %g significant test ",
                        "statistics (got %d); lower min_int to fit a smaller set."),
                 min_int, n_sig))
  }
...
#   date           = if (is.null(date)    || date    == "") NULL else date
  ...
  if (!is.null(yi)) {
    ...
    results$effect_size <- list(
      ...
      prediction_interval = res_main$prediction_interval,
  ...
  results$plot_control <- plot_control

  if (isTRUE(show_plot)) {
    if (!exists("plot.zcurve", mode = "function")) {
      stop("plot.zcurve() is unavailable. Load the package plotting file before calling zcurve().")
    }
    # xlim defaults to the modeling grid but plot_control may override it.
    plot_args <- utils::modifyList(list(xlim = c(x_lim_min, x_lim_max)), plot_control)
    do.call(plot, c(list(results), plot_args))
  }
...
#' Methods for zcurve objects
#'
#' `print` and `summary` methods for the objects returned by [zcurve()].
#'
#' @param x,object A `zcurve` (or `summary_zcurve`) object.
#' @param digits Number of significant digits to print.
#' @param ... Further arguments, currently ignored.
#' @return The `print` methods return their argument invisibly; `summary`
#'   returns a `summary_zcurve` object.
#' @seealso [zcurve()]
#' @name zcurve-methods
#' @export
...
summary.zcurve <- function(object, ...) {
  ...
  result <- list(
    ...
    prediction_interval = object$effect_size$prediction_interval,
...
print.summary_zcurve <- function(x, digits = max(3, getOption("digits") - 3), ...) {
  ...
  if (!is.null(x$prediction_interval)) {
    pi <- x$prediction_interval
    cat(sprintf("\nPrediction interval for a new study (95%% of population effects; mu = %.2f, tau = %.2f):\n",
                pi["mu"], pi["tau"]))
    if (is.finite(pi["lb"])) {
      cat(sprintf("  [%.2f, %.2f]   (heterogeneity + sampling error in mu and tau)\n",
                  pi["lb"], pi["ub"]))
    } else {
      cat(sprintf("  [%.2f, %.2f]   (heterogeneity only; set boot_iter > 0 to add sampling error)\n",
                  pi["lb_het"], pi["ub_het"]))
    }
  }
